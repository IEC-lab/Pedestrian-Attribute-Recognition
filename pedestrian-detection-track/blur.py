...
class BlurDetection:
    ...
    def preImgOps(self, imgName):
        ...
        # cv2.imshow("原始图", img)
        # 预处理操作
        reImg = cv2.resize(img, (100, 200), interpolation=cv2.INTER_CUBIC)  #
    ...
    def _drawImgFonts(self, img, strContent):
        ...
        cv2.putText(img, strContent, (0, 10), font, fontSize, (0, 255, 0), 2)
    ...
    def _lapulaseDetection(self, imgName):
        ...
        # strp3: 绘制图片并保存  不应该写在这里  抽象出来   这是共有的部分
        newImg = self._drawImgFonts(reImg, str(round(score,3)))
...
if __name__ == "__main__":
    BlurDetection = BlurDetection(strDir="/home/lyf/git-repo/RFSong-multidata/person/1/")
    st=time.time()
    BlurDetection.TestDect () # TestSMD
    print(time.time()-st)