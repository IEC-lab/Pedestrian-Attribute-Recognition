...
from models import getResNet, get_fsr, seg_cal
...
def train(args, ctx):
    ...
    # name_file = open('/home/lyf/git-repo/imbalanced_learning/wider_records/training_list.lst')
    # name_file_lst=[]
    # for line in name_file:
    #     nfl='.'+line.split()[-1][20:]
    #     name_file_lst.append(nfl)
    # path = "/home/lyf/git-repo/Self-Correction-Human-Parsing/seg/image.txt"
    ...
    # mx.viz.plot_network(model, shape={"data":(1,3,224,224)}).view()
    ...
    # Load Models
    model.load_params('models_8311/backbone_resnet.params', ctx=ctx, allow_missing=True, ignore_extra=True)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_id, batch in enumerate(train_iter):
            ...
            # # output_tensor_cls = output_dict[name_file_lst[st]][1]
            ...
            #     # output_tensor_lst=output_tensor_lst +(cpu_tensor)
            # # print(np.array(output_tensor_lst).shape)
            # seg_cal_fun=seg_cal(num_classes=args.num_classes, lst=np.array(output_tensor_lst),ctx=ctx)
            ...
            with autograd.record():
                ...
                # net_features_stg4_output=seg_cal_fun(net_features_stg4)
                # output=net_features_stg4_output
                # print(output)
                # print(output_org)
                # output=output_org
                # print(net_features_stg4_output.shape)
                
                # print(net_features_stg3_v1, net_features_stg3, net_features_stg4,output)
                # print(net_features_stg4.shape)
                all_stages = {}
                for stage in range(2, 5):
                    if stage == 2:
        ...
        print("train: ",train_mAP, train_APs, "val: ", val_mAP, val_APs, "best: ",best_mAP)
        # if epoch == 3:
        #     patience = 0
        ...
        if val_mAP > best_mAP:
            ...
            best_mAP = val_mAP
            for stage in range(2, 5):
                stages['stage_' + str(stage)].save_params('saved_models/joint_new_fsr_stage_' + str(stage) + '.params')
                stage_attentions['stage_' + str(stage)][0].save_params('saved_models/joint_new_fconv_stage_' + str(stage) + '.params')
                stage_attentions['stage_' + str(stage)][1].save_params('saved_models/joint_new_fatt_stage_' + str(stage) + '.params')
            if not args.finetune:
                model.save_params('saved_models/joint_new_base_resNet.params')
            print('Model Saved with mAP=%f' % best_mAP)
        else:
            patience += 1
            if patience > 6 and lr_drops < 3:
                lr_drops += 1
                lr_scheduler.learning_rate /= float(10)
                print("New Learning Rate=%f" % lr_scheduler.learning_rate)
                patience = 0
